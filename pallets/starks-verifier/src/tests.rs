...
fn should_create_task() {
	...
	ext.execute_with(|| {
		...
		assert_ok!(Verifier::create_task(
			Origin::signed(1),
			class,
			progam_hash,
			inputs.clone(),
		...
		assert_eq!(
			...
			Some(Status { verifiers: Vec::<u32>::new(), ayes: 0, nays: 0 })
		);
	})
}
...
fn should_parse_http_response() {
...

	{
		...
		let uri = "https://ipfs.infura.io:5001/api/v0/cat?arg=".to_owned() +
...
fn should_send_extrinsic() {
	...
	ext.execute_with(|| {
		...
		assert_ok!(<Verifier as sp_runtime::traits::ValidateUnsigned>::validate_unsigned(
		...
		assert_eq!(status, Some(Status { verifiers: vec![2], ayes: 1, nays: 0 }));
		// then
...
fn task_params() -> ([u8; 32], Vec<u128>, Vec<u128>, Vec<u8>) {
	(
		[
			19, 23, 145, 150, 7, 226, 183, 94, 42, 36, 220, 169, 148, 89, 125, 153, 113, 250, 202,
			142, 187, 167, 14, 144, 186, 217, 89, 214, 222, 234, 43, 214,
		],
		vec![1u128, 0u128],
		vec![8u128],
