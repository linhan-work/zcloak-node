...
impl Stack {
	...
	fn op_and(&mut self) {
		...
		self.registers[0][self.step] =
			if x == field::ONE && y == field::ONE { field::ONE } else { field::ZERO };
		self.shift_left(2, 1);
	}
	...
	fn op_cmp(&mut self, hint: OpHint) {
		...
		let next_power_of_two =
			if power_of_two == 1 { field::div(power_of_two, 2) } else { power_of_two >> 1 };
		...
		self.registers[6][self.step] =
			field::add(self.registers[6][self.step - 1], field::mul(b_bit, power_of_two));
		self.registers[7][self.step] =
			field::add(self.registers[7][self.step - 1], field::mul(a_bit, power_of_two));
