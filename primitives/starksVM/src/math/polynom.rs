...
mod tests {
	...
	fn eval() {
		...
		let poly: [u128; 4] =
			[384863712573444386, 7682273369345308472, 13294661765012277990, 16234810094004944758];
		...
		assert_eq!(
			field::add(poly[0], field::add(field::mul(poly[1], x), field::mul(poly[2], x2))),
			super::eval(&poly[..3], x)
		);
	...
	fn add() {
		...
		let pr = vec![field::add(poly1[0], poly2[0]), field::add(poly1[1], poly2[1]), poly2[2]];
		assert_eq!(pr, super::add(&poly1[..2], &poly2));

		// poly2 is lower degree
		let pr = vec![field::add(poly1[0], poly2[0]), field::add(poly1[1], poly2[1]), poly1[2]];
	...
	fn div() {
		...
		let poly2: Vec<u128> =
			vec![9918505539874556741, 16401861429499852246, 12181445947541805654];
