...
impl Evaluator {
	...
	pub fn evaluate_boundaries(&self, current: &TraceState, x: u128) -> (u128, u128) {
		...
		for i in 0..loop_stack.len() {
			...
			result_adj =
				field::add(result_adj, field::mul(loop_stack[i], cc.loop_stack[i * 2 + 1]));
		}
	...
	pub fn get_transition_degrees(&self) -> Vec<usize> {
		return [self.decoder.constraint_degrees(), self.stack.constraint_degrees()].concat()
	}
}
...
fn parse_program_hash(program_hash: &[u8; 32]) -> Vec<u128> {
	return vec![field::from_bytes(&program_hash[..16]), field::from_bytes(&program_hash[16..])]
}
