		...
		pub fn user_verify(
			...
			<TaskParams<T>>::insert(
				...
				UserTaskInfo {
					...
					is_task_finish: Some(UserTaskStatus::JustCreated),
				},
			);
			...
			let is_success =
				Self::stark_verify(&program_hash, inputs.clone(), outputs.clone(), &proof);
			...
			if res {
				<TaskParams<T>>::insert(
					...
					UserTaskInfo {
						...
						outputs,
						program_hash,
						is_task_finish: Some(UserTaskStatus::VerifiedTrue),
					},
				);
				...
				<TaskParams<T>>::insert(
					...
					UserTaskInfo {
						...
						outputs,
						program_hash,
						is_task_finish: Some(UserTaskStatus::VerifiedFalse),
					},
				);
