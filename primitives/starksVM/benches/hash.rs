...
pub fn gmimc(c: &mut Criterion) {
	...
	let mut r = [0u8; 32];
	c.bench_function("GMiMC", |bench| bench.iter(|| hash::gmimc(black_box(&v), black_box(&mut r))));
}
...
pub fn sha3(c: &mut Criterion) {
	...
	let mut r = [0u8; 32];
	c.bench_function("Sha3", |bench| bench.iter(|| hash::sha3(black_box(&v), black_box(&mut r))));
}
