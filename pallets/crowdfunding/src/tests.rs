use super::*;
use crate::{mock::*, Error, Error::CreateAssetFail};
use frame_support::{
	assert_noop, assert_ok,
	traits::{ExistenceRequirement::AllowDeath, OffchainWorker, OnFinalize},
};
use pallet_starks_verifier::{Status, TaskInfo, TaskStatus};
use sp_core::{
	offchain::{
...
fn create_asset_and_crowdfunding() {
	new_test_ext().execute_with(|| {
		...
		let CrowfundingStatus { is_funding_proceed, total_funding, .. } =
			StarksCrowdfunding::crowdfunding_process(1001);
		assert_eq!(is_funding_proceed, None);

		let crowdfunding_result = StarksCrowdfunding::create_crowdfunding(
			Origin::signed(1),
			CreateAsset,
			1001,
			2,
		...
		assert_eq!(
			Assets::balance(1001, pallet_admin),
			(4000_000_000_000_000 / CrowdFundingMetadataDepositBase::get())
		);
		...
		assert_eq!(
			total_funding,
			4000_000_000_000_000 - MinBalance::get() * CrowdFundingMetadataDepositBase::get()
		);
	});
}
...
fn transfer_asset_and_crowdfunding() {
	new_test_ext().execute_with(|| {
		...
		let mint_asset_to_1 = Assets::mint(
			Origin::signed(1),
			1001,
			1,
			(4000_000_000_000_000 / CrowdFundingMetadataDepositBase::get()),
...
fn create_asset_and_crowdfunding_again() {
	new_test_ext().execute_with(|| {
		...
		let crowdfunding_result2 = StarksCrowdfunding::create_crowdfunding(
			Origin::signed(2),
			CreateAsset,
			1001,
			4,
...
fn transfer_asset_but_not_have_enough_asset() {
	new_test_ext().execute_with(|| {
		...
		let mint_asset_to_1 = Assets::mint(
			...
			1001,
			1,
			(2000_000_000_000_000 / CrowdFundingMetadataDepositBase::get()),
		);
		assert_eq!(mint_asset_to_1, Ok(()));
		assert_eq!(
			(Assets::balance(1001, 1)),
...
fn crowdfunding_start_buyer_buyZtoken_without_KYC() {
	new_test_ext().execute_with(|| {
		...
		let buy_result =
			StarksCrowdfunding::buy_ztoken(Origin::signed(3), 1001, 1000_000_000_000_000);
