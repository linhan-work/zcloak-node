...
pub fn add128(c: &mut Criterion) {
	let x = field::rand();
	let y = field::rand();
	c.bench_function("add128", |bench| bench.iter(|| field::add(black_box(x), black_box(y))));
}

pub fn mul128(c: &mut Criterion) {
	let x = field::rand();
	let y = field::rand();
	c.bench_function("mul128", |bench| bench.iter(|| field::mul(black_box(x), black_box(y))));
}
...
pub fn exp128(c: &mut Criterion) {
	let x = field::rand();
	let y = field::rand();
	c.bench_function("exp128", |bench| bench.iter(|| field::exp(black_box(x), black_box(y))));
}
